...
async def get_banner(request: Request, response: Response, 
    ...
    if not "banner" in app.config.enabled_plugins:
    ...
    if highest in app.roles.keys():
        highest_role = app.roles[highest]
    ...
    try:
        r = await arequests.post("http://127.0.0.1:8700/banner", data=json.dumps({"company_abbr": app.config.abbr, \
